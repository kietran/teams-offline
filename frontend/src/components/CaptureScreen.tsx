...
type Props = { channels: Channel[]; chrome: ChromeState; capture: CaptureRun | null; busy: boolean; error: string | null; onManage: () => void; onStart: () => void; onPause: () => void; onResume: () => void; onRetry: (channelId: string) => void }

export function CaptureScreen({ channels, chrome, capture, busy, error, onManage, onStart, onPause, onResume, onRetry }: Props) {
  ...
  const fileTotals = (capture?.channels || []).reduce(
    (totals, item) => ({
      captured: totals.captured + item.files_captured,
      failed: totals.failed + item.files_failed,
      pending: totals.pending + item.files_pending,
    }),
    { captured: 0, failed: 0, pending: 0 },
  )
  return <main className="page" id="main-content" tabIndex={-1}>
    ...
    {active ? <section className="progress-panel" aria-labelledby="progress-title"><div><span id="progress-title">Đang lưu kênh {Math.min(completedChannels + 1, capture.channels.length)}/{capture.channels.length}</span><h2>{current?.displayName || 'Đang chuẩn bị…'}</h2></div>
      ...
      <p>{done}/{seen || '…'} bài viết · {capture.channels.reduce((sum, item) => sum + item.captured_replies, 0)} bình luận · {fileTotals.captured} tệp đã tải{fileTotals.failed ? ` · ${fileTotals.failed} tải lỗi` : ''}{fileTotals.pending ? ` · ${fileTotals.pending} chưa thử` : ''}</p></section> :
